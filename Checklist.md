...
> This is the deterministic math module implementing the real estate calculations per spec. :contentReference[oaicite:9]{index=9}
...
- [~] Create unit tests with fixed input cases:
  - [~] Reproduce spreadsheet outputs for at least 3 scenarios:
    - Simple baseline, high rehab, high leverage.
  - [~] Verify key metrics (cash flow, equity, total return, DSCR, etc.).
...
- [x] Quality-grade multiplier (A/B/C) applied across line items.
- [x] Add quality-grade multiplier (A/B/C) to scale line-item unit costs.
...
- [x] Set up core routes:
  - [x] `/` – landing page.
  - [x] `/analyze` – main analysis view (strategy + tabs).
  - [x] `/analysis/[id]` – load specific saved scenario.
  - [x] `/s/[token]` – shared read-only view.
  - [x] `/admin` – admin dashboard.
  - [x] `/admin/properties` – property DB management.
...
- [x] Add per-field tooltips/help text.
...
- [~] Implement:
  - [x] Manual “Recalculate” button (V1).
  - [x] Optional debounced auto-update (later).
...
- [~] Implement optional MonthlyScheduleView:
  - [x] Collapsible advanced section for month-by-month data (snapshot table).
...
- [x] Implement “Save analysis” button:
  - [x] Creates or updates analysis via API.
- [x] Implement “Save As / Duplicate” functionality:
  - [x] Calls `POST /api/analyses` with input_payload cloned from current.
- [x] Implement “My Analyses” list page:
  - [x] Table of saved analyses (name, strategy, property, date).
  - [x] Actions: open, duplicate, delete.
...
- [x] Implement route `/s/[token]`.
- [~] On load:
  - [~] Fetch share data via `GET /api/shares/:token`.
- [~] Render:
  - [x] Read-only summary of inputs and outputs.
